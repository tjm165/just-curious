...
resource "null_resource" "compile" {
  ...
  provisioner "local-exec" {
    ...
    command = var.runtime == "go1.x" ? "GOOS=linux GOARCH=amd64 go build -o lambda main.go" : "echo pass for python"
  }
}
...
# if the runtime is "go1.x" then need to change the handler
